...
class VideoDownloader(QMainWindow):
    ...
    def setup_youtube_tab(self):
        ...
        warning_text = QLabel("⚠️ FFmpeg is not installed. FFmpeg is required for the video download process.")
        warning_text.setStyleSheet("color: #ffffff; font-size: 13px;border: 1px solid #404040;")
        banner_layout.addWidget(warning_text)
        
        install_button = QPushButton("Install FFmpeg")
        ...
        # Başlık
        title_label = QLabel("Download YouTube Video")
        ...
        # Alt başlık
        subtitle_label = QLabel("Paste the YouTube link to download the video.")
        ...
        self.url_input.setPlaceholderText("Copy the YouTube video link then press the 'Paste' button.")
        ...
        self.download_thumbnail_btn = QPushButton("Download the Thumbnail")
        ...
        self.download_button = QPushButton("Download The Video")
        ...
        self.download_eta = QLabel("Estimated Time: --:--:--")
    ...
    def setup_web_video_tab(self):
        ...
        self.web_url_input.setPlaceholderText("Copy the URL of the video and click the 'Paste' button.")
        self.web_paste_button = QPushButton("Paste")
        url_layout.addWidget(self.web_url_input)
        url_layout.addWidget(self.web_paste_button)
        layout.addWidget(url_group)
        
        # İndirme butonu
        self.web_download_button = QPushButton("Download")
    ...
    def setup_codec_tab(self):
        ...
        self.codec_file_input.setPlaceholderText("Select a video file.")
        ...
        audio_codec_label = QLabel("🔊  Sound Codec:")
        ...
        acceleration_label = QLabel("Acceleration Mode:")
        ...
        self.conversion_time = QLabel("⏱️ Time: --:--:--")
        self.conversion_eta = QLabel("🕒 Remaining time: --:--:--")
    ...
    def setup_history_tab(self):
        ...
        self.history_table.setHorizontalHeaderLabels([
            "Date", "File Name", "URL", "Status", "File Path"
        ])
    ...
    def update_history_buttons(self):
        ...
        if has_selection:
            ...
            if not file_exists:
                ...
                self.history_table.item(row, 3).setText("File Not Found!")
    ...
    def paste_url(self):
        ...
        try:
            ...
            self.paste_button.setText("In progress...")
    ...
    def on_url_changed(self):
        ...
        if url:
            try:
                ...
                # Butonu güncelle
                self.paste_button.setText("Retrieving Data...")
                self.paste_button.setEnabled(False)
    ...
    def update_video_info(self, info):
        ...
        try:
            ...
            # Kanal adı
            channel = info.get('uploader', 'Channel Not Found')
            ...
            if duration:
                ...
                self.duration_label.setText(f"⏱️  Time: {duration_str}")
            else:
                self.duration_label.setText("⏱️  Time: Unknown")
            ...
            if views >= 1000000:
                ...
                views_str = str(views)
            self.views_label.setText(f"👁️  Views: {views_str}")
            ...
            if upload_date:
                try:
                    ...
                    self.upload_date_label.setText(f"📅  Release Date: {formatted_date}")
                except:
                    self.upload_date_label.setText(f"📅  Release Date: Unknown")
            else:
                self.upload_date_label.setText("📅  Release Date: Unknown")
        ...
        if hasattr(self, 'current_thumbnail_url'):
            try:
                ...
                if response.status_code == 200:
                    ...
                    QMessageBox.warning(self, "Error", f"Thumbnail couldn't be downloaded! HTTP Code: {response.status_code}")
            except Exception as e:
                QMessageBox.warning(self, "Error", f"Thumbnail couldn't be downloaded: {str(e)}")
    ...
    def start_download(self, mp3_only=False):
        try:
            ...
            if mp3_only:
                ...
                self.download_button.setText("Downloading Video...")
                selected_items = self.format_table.selectedItems()
                if not selected_items:
                    QMessageBox.warning(self, "Warning", "Please select a video format.")
        ...
        try:
            if d['status'] == 'downloading':
                ...
                if total:
                    ...
                    self.download_percent.setText(f"Progress: %{percent:.1f}")
                
                # Boyut bilgisi
                if total:
                    if total > 1024*1024*1024:  # GB
                        size_str = f"Size: {downloaded/1024/1024/1024:.1f} GB / {total/1024/1024/1024:.1f} GB"
                    else:  # MB
                        size_str = f"Size: {downloaded/1024/1024:.1f} MB / {total/1024/1024:.1f} MB"
                else:
                    if downloaded > 1024*1024*1024:  # GB
                        size_str = f"Size: {downloaded/1024/1024/1024:.1f} GB"
                    else:  # MB
                        size_str = f"Size: {downloaded/1024/1024:.1f} MB"
                ...
                try:
                    ...
                    if eta:
                        if isinstance(eta, str):
                            self.download_eta.setText(f"Remaining Time: {eta}")
                        else:
                            minutes = int(eta // 60)
                            seconds = int(eta % 60)
                            self.download_eta.setText(f"Remaining Time: {minutes:02d}:{seconds:02d}")
                    else:
                        self.download_eta.setText("Remaining Time: --:--")
                except:
                    self.download_eta.setText("Remaining Time: --:--")
    ...
    def on_download_complete(self, downloaded_file):
        ...
        try:
            if downloaded_file:
                ...
                QMessageBox.information(self, "Success", "Download is Finished!")
        except Exception as e:
    ...
    def convert_to_mp3_with_libshine(self, input_file):
        ...
        try:
            ...
            QMessageBox.warning(self, "Conversion Error", f"MP3 convertion failed: {str(e)}")
            return None
    ...
    def update_ffmpeg_status(self):
        ...
        try:
            ...
            if result.returncode == 0:
                # FFmpeg bulundu
                self.ffmpeg_status.setText("✅  FFmpeg installed and ready.")
                ...
                # FFmpeg bulunamadı
                self.ffmpeg_status.setText("❌  FFmpeg is not installed on your system")
                self.ffmpeg_status.setStyleSheet("""
                    QLabel {
                        color: #f44336;
                        font-size: 13px;
            ...
            # Hata durumunda
            self.ffmpeg_status.setText("❌  FFmpeg status check failed.")
            self.ffmpeg_status.setStyleSheet("""
                QLabel {
                    color: #f44336;
                    font-size: 13px;
    ...
    def download_ffmpeg(self):
        ...
        try:
            ...
            progress_dialog.setWindowTitle("Installing FFmpeg...")
            ...
            info_label = QLabel("FFmpeg installing with winget ...")
            ...
            status_label = QLabel("Installation is starting...")
            ...
            self.ffmpeg_status.setText("FFmpeg is installing...")
            ...
            if result.returncode == 0:
                ...
                QMessageBox.information(
                    self, 
                    "Success", 
                    "FFmpeg is successfully installed on your system. You can use the program now!"
                )
            else:
            ...
            if 'progress_dialog' in locals():
                progress_dialog.close()
                progress_dialog.deleteLater()
            QMessageBox.warning(self, "Error", f"FFmpeg installation is failed: {str(e)}")
        finally:
    ...
    def save_settings(self):
        ...
        try:
            ...
            QMessageBox.information(self, "Success", "Settings are saved!")
            
        except Exception as e:
            QMessageBox.warning(self, "Hata", f"Settings aren't saved: {str(e)}")
    ...
    def cancel_download(self):
        ...
        try:
            reply = QMessageBox.question(
                self, 
                'Cancel the Download!',
                'Are you sure to cancel the download?',
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
                QMessageBox.StandardButton.No
            )
            ...
            if reply == QMessageBox.StandardButton.Yes:
                ...
                self.reset_download_state()
                QMessageBox.information(self, "Canceled", "Download is canceled and the remaining files are deleted!")
    ...
    def update_conversion_progress(self, progress_dict):
        ...
        try:
            ...
            # Detayları güncelle
            self.conversion_speed.setText(f"Speed: {progress_dict['speed']} fps")
            self.conversion_time.setText(f"Time: {progress_dict['time']}")
            self.conversion_eta.setText(f"Remaining Time: {progress_dict['eta']}")
            self.conversion_percent.setText(f"Progress: %{progress_dict['percent']:.1f}")
    ...
    def conversion_finished(self):
        ...
        QMessageBox.information(self, "Success", "Convertion is completed!")
        self.convert_button.setEnabled(True)
        self.codec_progress.setValue(0)
        self.convert_button.setText("Convert")

        # Detayları güncelle
        self.conversion_speed.setText(f"Speed: -- fps")
        self.conversion_time.setText(f"Time: --:--:--")
        self.conversion_eta.setText(f"Remaining Time: --:--:--")
        self.conversion_percent.setText(f"Progress: %0")
        

    def conversion_error(self, error):
        """Dönüştürme hatası olduğunda çağrılır"""
        QMessageBox.critical(self, "Convertion Error", str(error))
        self.convert_button.setEnabled(True)
        self.codec_progress.setValue(0)
    ...
    def setup_settings_tab(self):
        ...
        info_label = QLabel("Manage Download Location and FFmpeg status")
        info_label.setStyleSheet("color: #b0b0b0; font-size: 12px;")
        text_layout.addWidget(info_label)
        ...
        self.ffmpeg_download_button = QPushButton("Download and Install FFmpeg")
        ...
        quality_group = QGroupBox("⚙️  Default Quality Settings")
        ...
        audio_quality_label = QLabel("🔊  Sound Quality:")
    ...
    def run(self):
        try:
            ...
            if process.wait() == 0 and self.is_running:
                ...
                raise Exception("Convertion is failed.")
